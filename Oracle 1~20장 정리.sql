...
2.3_논리 연산자
2.4_BETWEEN AND 연산자
2.5_IN 연산자를 이용한 조건 검색
2.5+_NOT IN 연산자
2.6_LIKE 연산자와 와일드 카드
2.6+_NOT LIKE 연산자
2.7_NULL을 위한 연산자 IS NULL 과 IS NOT NULL
2.7+_NULL 연산자를 이용한 조건 검색
2.8_정렬을 위한 ORDER BY 절
...
========================== 2.4_BETWEEN AND 연산자 ============================
    ...
    BETWEEB AND 연산자는 특정 칼럼의 데이터 값이
    하한 값(A)와 상한값(B) 사이에 포함되는 로우를 검색하기 위한 연산자
...
===================== 2.5_IN 연산자를 이용한 조건 검색 ======================
    ...
    IN 연산자는 특정 칼럼의 값이 A,B,C 중에 하나라도 일치하면 참이 되는 연산자
...
*/
-- 커미션이 300 이거나 500 이거나 1400 인 사원을 검색하기 위해서는 
-- OR 연산자를 활용했지만 IN 연산자도 가능

SELECT *
FROM EMP
WHERE COMM = 300 OR COMM = 500 OR COMM = 1400;

-- OR 연산자로 묶인 비교 연산자 내의 칼럼이 이처럼 COMM 으로 동일할 때
-- IN 연산자를 사용할 수 있음
...
======================= 2.6_LIKE 연산자와 와일드 카드 ===========================
    ...
    LIKE 연산자는 칼럼에 저장된 문자 상수 중에서
    LIKE 연산자에서 지정한 문자 패턴과 부분적으로 일치하면 참이 되는 연산자
    
    예를들어 성이 '이' 씨인 사람을 찾거나
    거주지가 '서울' 인 사람을 찾는것과 같이
    
    칼럼에 저장된 데이터의 일부만 일치하더라도 조회가 가능하도록 하기 위해 사용
    
    LIKE 연산자는 pattern 을 기술해야함.

    <형식>

column_name LIKE pattern;

    pattern에는 두 가지 와일드카드가 사용됨.
    
    와일드카드 덕분에 임의의 문자 또는 문자열을 포함한 값을 찾을 수 있음

    LIKE 연산자는 검색하고자 하는 값을 정확히 모를 경우에
    와일드카드와 함께 사용하여 원하는 내용을 검색할 수 있음
    
    사원 테이블 중 F로 시작하는 이름을 갖는 사원을 검색하고자 할 때
    다른 값을 정확히 모를경우
    즉, 특정 문자가 포함되기만 하고 그 이전이나 이후에 어떤 문자가 몇 개가 오든지
    상관없다는 의미를 표현하기 위해서는 LIKE 연산자와 함께 '%'를 사용
    
    사원 이름이 'F' 로 시작하는 사원을 출력하기 위한 표현을
    ename = 'F'로 검색하게 되면 
    ...
    '_' (언더바) 기호는 단 한 문자에 대해서만 와일드카드 역할을 함.
    '_' (언더바) 를 기술한 위치에 어떤 문자가 오든 상돤없다는 의미로 사용됨.
    그렇기 때문에 '_' 기호는 사용되는 위치와 순서가 중요함!
    
    이름의 두번 째 글자가 'A' 인 사원을 검색할면 '_' 와일드카드를 사용해서
    "_A" 와 같이 표현 해야함.
    
    '_' (언더바) 를 'A' 앞에 기술하여 첫 번째에 어떤 문자가 오든 상관없게 하고
    'A' 뒤에는 무엇이 오든 상관없어야 하기 때문에 '%'를 기술함.
    
*/
-- 사원들 중에서 이름이 F로 시작하는 사람만 찾아보기.

SELECT *
FROM EMP
WHERE ENAME LIKE 'F%';

-- 사원들 중에서 이름이 J로 시작하는 사람만 찾아보기

SELECT *
FROM EMP
WHERE ENAME LIKE 'J%';

-- 이름 중 A를 포함하는 사원을 검색하기

SELECT *
FROM EMP
WHERE ENAME LIKE '%A%';

-- 문자열의 앞에 어떤 문자열이 몇 개가 오든 상관없이 N으로 끝나는 데이터 찾아보기
...
====================== 2.7_NULL을 위한 연산자 IS NULL 과 IS NOT NULL ======================
    ...
    오라클에서 NULL 값이 저장되는 것을 허용하지만,
    NULL은 매우 특이한 성격을 가진 값이기 때문에
    NULL 값을 제대로 이해하지 못하면 원하는 결과를 얻지 못할 수 있음
    
    NULL에 대한 이해와 함께 반드시 알아야 할 내용은
    NULL을 위한 연산자.
    
    NULL에 대한 처리를 위해서 NULL 값이 저장된 칼럼부터 살펴보기.
    
    사원 테이블의 COMM 칼럼에 NULL 이 저장되어 있으므로
    COMM 칼럼의 성격을 파?老歐 위해서
    사원의 이름과 소속 부서번호와 커미션을 출력해보기.
*/    
...
/*
-- 30번 부서 소속 사원인 경우에만 커미션을 받을 수 있다는 것을 알 수 있음.

 ========================= 2.7+_NULL 연산자를 이용한 조건 검색 =========================

    '=' 연산자로 커미션을 받지 않는 사원에 대해 검색해보기.
    칼럼 값이 NULL 인지 검색하기 위해서 '=' 를 사용해서 연산을 하게 되면
    원하는 결과를 얻을 수 없음.
    
    왜냐하면 NULL이 저장되어 있는 경우에는 '=' 연산자로 판단할 수 없기 때문.
...
/*  NULL 값은 이러한 문제점을 갖고 있기 때문에 
    특정 칼럼에 NOT NULL 이라는 제약 조건을 주어서
    NULL 값을 저장할 수 없도록 제한을 두기도 하고
    NULL 값에 대하 연산을 위해서 특별한 연산자가 제공됨.
    
    미확정, 알 수 없는(UNKNOWN) 값을 의미하는 NULL은
    연산, 할당, 비교가 불가능
    
    오라클에서는 NULL 값을 가진 데이터를 비교하기 위한 연산자로
    'IS NULL'을 제공함
    
    'IS NULL' 연산자는 칼럼 값 중에 NULL 을 포함하는 로우를 검색하기 위해 사용
    
*/

--  IS NULL을 사용하면 커미션을 받지 않는 사원을 검색할 수 있음

SELECT *
FROM EMP
WHERE COMM IS NULL;

-- IS NULL 연산자 사이에 NOT 을 기술한 IS NOT NULL 연산자는 
-- 칼럼 값이 NULL 이 아닌 로우를 검색하는 경우에 사용하는 연산자.
...
======================== 2.8_정렬을 위한 ORDER BY 절 ==========================
    ...
    SELECT  문으로 검색하게 되면 테이블에 데이터가 입력된 순서대로 출력됨.
    하지만, 데이터의 출력 순서를 특정 칼럼을 기준으로 순서대로 나열할 필요가 있음.
    
    이렇게 데이터를 정렬해야 하는 경우에 ORDER BY 절을 사용.
...
ORDER BY column_name sorting
    ...
    정렬은 특정 칼럼을 기준으로 하기 때문에 ORDER BY 절 다음에는
    기준이 될 칼럼명을 기술함.
    
    정렬 방식에는 작은 것이 위에 출력되고,
    아래로 갈수록 큰 값이 출력되도록 하는 오름차순(Ascending) 정렬방식 과
    큰 값이 위에 출력되고 아래로 갈수록 작은 값이 출력되도록 하는 내림차순(Descending)정렬 방식이 있음
    ...
    수치 데이터가 아닌 문자 데이터에 대해서 정렬을 하게 되면
    오름차순인 경우에는 A,B...Z 순으로 출력되고
    내림차순인 경우에는 Z,Y...A 순으로 출력됨
    
    문자 데이터의 경우 아스키 코드 값으로 저장되므로 아스키 코드 값을 기준으로 정렬
    
    날짜 데이터에 대해서 오름차순으로 지정하면
    가장 오래된 과거의 시점이 가장 위에 출력되고 아래로 갈수록 최근 시점이 출력됨
                       
                       내림차순으로 지정하면 최근 시점부터 출력됨.
*/
